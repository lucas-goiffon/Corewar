...
void lldi_operation(info_t *info, champ_t *ch, op_t op, int *params)
{
    ...
    sum = take_param_lldi(info, ch, params[0], &i);
    i = verify_pc_wc(i);
    sum += take_param_lldi(info, ch, params[1], &i);
    i = verify_pc_wc(i);
    if (info->memory[i] < 1 || info->memory[i] > REG_NUMBER)
        return;
    j = (ch->pc + sum) % MEM_SIZE;
    pc_sum = verify_pc_wc(ch->pc + sum);
...
int lldi_inst(info_t *info, champ_t *ch)
{
    ...
    if (info->memory[ch->pc] != op.code)
        return (0);
    if (set_cycle_lldi(info, ch, op) == 1)
        return (1);
    params = take_parameters(info, ch, op);
    if (verify_parameters(info, ch, op, params) == 84)
        return (0);
    lldi_operation(info, ch, op, params);
    lldi_change_pc(info, ch, params, op);
    free(params);
    return (1);
}