...
void vm_initialization(info_t *info)
{
    ...
    if ((info->music = sfMusic_createFromFile("sounds/music.ogg")) == NULL)
        my_exit(info, 84, "Problem with sounds/music.ogg");
    sfMusic_setVolume(info->music, 50.0);
    if ((info->die_sound = sfMusic_createFromFile("sounds/die.ogg")) == NULL)
        my_exit(info, 84, "Problem with sounds/die.ogg");
