...
int set_str_or_operation(info_t *info, unsigned char *str, int param, int j)
{
    if (param == 3) {
        str[0] = 0;
        str[1] = 0;
...
void or_operation(info_t *info, champ_t *ch, op_t op, int *params)
{
    ...
    i = set_str_or_operation(info, one, params[0], i) % MEM_SIZE;
    if (params[1] == 1)
        u_strcpy(two, ch->registers[info->memory[i + 1] - 1], 4);
    i = set_str_or_operation(info, two, params[1], i + 1) % MEM_SIZE;
...
int or_inst(info_t *info, champ_t *ch)
{
    ...
    if (ch->pc < 0 || ch->pc >= MEM_SIZE)
        return (1);
    if (info->memory[ch->pc] != op.code)
        return (0);
    if (set_cycle_or(info, ch, op) == 1)
        return (1);
    params = take_parameters(info, ch, op);
    if (verify_parameters(info, ch, op, params) == 84)
        return (1);
    or_operation(info, ch, op, params);
    or_change_pc(info, ch, params, op);
    free(params);
    return (1);
}